...
module.exports.getAllPatients = async(req, res) => {
        try {
            ...
            //if patientData is empty, return a message
            if (patientData.length === 0) return res.status(404).json({ message: 'No patients found' });
...
module.exports.updateDoctor = async(req, res) => {
        const updater = await Staff.findById({ _id: req.params.updaterId })
        if (!updater) return res.status(400).json('Personnel inexistant')
        const doctor = await Staff.findById({ _id: req.params.doctorId })
